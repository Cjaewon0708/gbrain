 ...
 * the collision guard below must still reject distinct DB slugs such as FOO
 * and foo that fold to the same Windows/macOS filename.
 */
function resolveExistingPathCasing(targetPath: string): string {
  ...
  for (const [index, part] of parts.entries()) {
    if (index === parts.length - 1) {
      return current ? join(current, part) : part;
    }
