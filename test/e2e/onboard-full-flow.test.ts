...
describe('onboard E2E — runAllOnboardChecks', () => {
  ...
  test('empty brain: stale/link/timeline/takes checks are ok when take bootstrap is off', async () => {
    const results = await runAllOnboardChecks(engine);
    ...
    expect(byName.takes_count).toBe('ok'); // take bootstrap defaults to disabled
  });
  ...
  test('empty brain remediations: takes_count gated, pack_upgrade_available may surface', async () => {
    ...
    // takes_count is ok and emits no remediation because takes.bootstrap_enabled
